...
let storage = multer.diskStorage({ //# diskStorage => 파일이 저장될 경로와 파일명을 지정
...
router.post("/", multerUpload.single("txtFile"), async (req, res, next) => {
  ...
  try {
      ...
      let data = "";
      for (let k = 0; k < 5; k++) { // Core 이동
          for (let j = 0; j < 5; j++) { // Task 이동
              for (let i = 0; i < 10; i++) { //Case 이동
                  data += arr[i + (i * 24) + j + (k * 5)] + " "; //case 10개에 대한 각 Core의 하나의 Task에 대한 수행 능력을 문자열로 저장
              } //"case1 case2 case3 case4 case5 case6 case7 case8 case9 case10"
              if (j == 0) { //각 행의 첫 열만 INSERT 나머지는 UPDATE => 각 Core의 Task1만 INSERT 나머지는 UPDATE
...
router.get("/:index/:graph/:fileName", async (req, res, next) => { //DB에서 데이터 조회 및 그래프 그리기
    ...
    else { //Core1~5 버튼 클릭 시 Core의 task별 그래프
        xLabel = "task";
        subject = "Core"
    }
